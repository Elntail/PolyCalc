...
public class PolyTest{
    ...
    //Rommel's 5 tests
    /**
     * Tests polynomial's Add, Multiply, Division.
     * Assumes the following methods are correct: constructor, add(), multiply(), division().
     */
    @Test
    public void test4AddMultDiv(){

        Poly p1 = new Poly(2.5, 2); //p1 = 2.5x^2
        Poly p2 = new Poly(5, 2); //p2 = 5x^2
        Poly p3 = new Poly(2,0); // p3 = 2
        p1 = p1.add(p1).multiply(p2).divide(p3); //p1 = 12.5x^2
        Poly p4 = new Poly(12.5,4);// p4 = 12.5x^2

        assertEquals("12.5x^4 != 12.5x^2", p1, p4);

        p1 = new Poly();
        p1 = p1.multiply(p2).divide(p3);//p1 = 

        assertNotEquals("null == 12.5x^2", p1, p3);

        p1 = new Poly(2, 2);
        p1 = p1.add(new Poly(5, 5)).add(new Poly(4, 4)).add(new Poly(3, 3)).add(new Poly(1, 1));
        //p1 = 5x^5 + 4x^4 + 3x^3 + 2x^2 + x 

        p2 = p1; //p2 = 5x^5 + 4x^4 + 3x^3 + 2x^2 + x 
        p3 = p1.divide(p2); //p3 = 1;

        p4 = new Poly(1,0);
        assertEquals("1 != 1", p3, p4);

    }

    /**
     * Tests polynomial's add() and checks if the final size of poly = 10000.
     * Assumes the following methods are correct: constructor, add().
     */
    @Test
    public void testAddto10000(){
        Poly p1 = new Poly();
        assertEquals("Poly is empty", 0, p1.poly.size());

        for (int i=1; i <= 10000; i++){ // add an element at a time, and poly list size increases
            p1 = p1.add(new Poly(i,i));
            assertEquals("Poly size doesn't match number of added elements", i, p1.poly.size());
        }

    }

    /**
     * Tests polynomial's Add and Subtract methods can cancel it out
     * Assumes the following methods are correct: constructor, add(), subtract().
     */
    @Test
    public void test2OriginalAddSub(){
        Poly p1 = new Poly(5,4);//p1 = 5x^4

        Poly p2 = new Poly(4, 4);//p2 = 4x^4

        Poly p3 = new Poly(5,4);//p3 = 5x^4

        //checks addition and subtraction
        p1 = p1.subtract(p2); // p1 =  x
        p1 = p1.add(p2); // p1 = 5x^4

        assertEquals("5x^4 != 5x^4", p1, p3);

        p1 = new Poly(3,4); // p1 = 3x^4
        p3 = new Poly(3,4); //p3 =  3x^4

        //checks long polynomials

        //checks add/subtract
        p1 = p1.add(new Poly(5,8)).add(new Poly(2,2)); // p1 = 5x^8 + 3x^4 + 2x^2
        p3 = p3.add(new Poly(5,8)).add(new Poly(2,2));

        p1 = p1.subtract(p3);// p1 = 0

        p1 = p1.add(p3); // p1 = 5x^8+ 3x^4 +2x^2

        assertEquals("5x^8+ 3x^4 +2x^2 != 5x^8+ 3x^4 +2x^2^4", p1, p3);

    }

    /**
     * Tests polynomial's multiply and divide methods cancel each other out
     * Assumes the following methods are correct: constructor, multiply(), divide().
     */
    @Test
    public void test2OriginalMultDiv(){
        Poly p1 = new Poly(5,4);//p1 = 5x^4
        ...
        p1 = p1.divide(p2); // p1 = 1.25
        p1 = p1.multiply(p2); // p1 = 5x^4
        ...
        p1 = p1.multiply(p3);// p1 = 25x^16+30x^12+20x^10+9x^8+12x^6+4x^4

        p1 = p1.divide(p3); // p1 = 5x^8+ 3x^4 +2x^2
    ...
    /**
     * Tests all polynomial's main methods
     * Assumes the following methods are correct: constructor, multiply(), divide(), add(), subtract().
     */
    @Test
    public void test5MultDivAddSub(){
        Poly p1 = new Poly(2.5, 2); //p1 = 2.5x^2
        Poly p2 = new Poly(5, 2); //p2 = 5x^2
        Poly p3 = new Poly(2,0); // p3 = 2
        Poly p4 = new Poly(5,6); // p4 = 5x^6

        p1 = p1.add(p1).multiply(p2).divide(p3).subtract(p4); //p1 = -5x^6 + 12.5x^4 
        Poly p5 = new Poly();
        p5 = p5.add(new Poly(12.5, 4)).add(new Poly(-5,6)); // p5 = -5x^6 + 12.5x^4

        assertEquals("-5x^6 + 12.5x^2 != -5x^6 + 12.5x^2", p1, p5);

        p1 = new Poly();
        p1 = p1.multiply(p2).divide(p3).subtract(p2); //p1 = null;

        assertNotEquals("5x^2 == 12.5x^2", p1, p2);
